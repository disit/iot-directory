   ...
   function checkLdapRole($connection, $userDn, $role) 
   {
            ...
            if($ds && $bind)
            {
                if(checkLdapMembership($ds, $ldapUsername, $ldapTool))
                {
                   if(checkLdapRole($ds, $ldapUsername, "ToolAdmin"))
                   {
                      ...
                      mysqli_close($link);
                      header("location: contextBroker.php");
                   }
                   else
                   {
	...
	else if(isset($_POST['addDevice']))
    {
        ...
        if(isset($_SESSION['loggedRole']))
        {
			...
			$q = "INSERT INTO iotdirectorydb.devices(name, type, contextBroker,  kind, protocol, format, created, macaddress, model, producer, latitude, longitude) " .
                 "VALUES('$name', '$type', '$contextbroker', '$kind', '$protocol', '$format', '$created', '$macaddress', '$model', '$producer', '$latitude', '$longitude')";
            $r = mysqli_query($link, $q);
	...
	else if(isset($_POST['updateDevice']))
    {
        ...
        if(isset($_SESSION['loggedRole']))
        {
			...
			$q = "UPDATE iotdirectorydb.devices SET name='$nameM', type='$typeM', kind= '$kindM',contextBroker='$contextbrokerM', uri='$uriM', protocol='$protocolM', format='$formatM', macaddress='$macaddressM', model='$modelM', producer='$producerM', latitude='$latitudeM', longitude='$longitudeM' WHERE name='$nameM'";
            $r = mysqli_query($link, $q);

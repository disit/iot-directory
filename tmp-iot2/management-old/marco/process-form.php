   ...
   function checkLdapRole($connection, $userDn, $role) 
   {
	...
	else if(isset($_POST['addDevice']))
    {
        ...
        if(isset($_SESSION['loggedRole']))
        {
			...
			$q = "INSERT INTO iotdirectorydb.devices(name, type, contextBroker,  protocol, format, created, macaddress, model, producer, latitude, longitude) " .
                 "VALUES('$name', '$type', '$contextbroker',  '$protocol', '$format', '$created', '$macaddress', '$model', '$producer', '$latitude', '$longitude')";
            $r = mysqli_query($link, $q);
	...
	else if(isset($_POST['updateDevice']))
    {
        ...
        if(isset($_SESSION['loggedRole']))
        {
			...
			$q = "UPDATE iotdirectorydb.devices SET name='$nameM', type='$typeM', contextBroker='$contextbrokerM', uri='$uriM', protocol='$protocolM', format='$formatM', created='$createdM', macaddress='$macaddressM', model='$modelM', producer='$producerM', latitude='$latitudeM', longitude='$longitudeM' WHERE name='$nameM'";
            $r = mysqli_query($link, $q);

                        ...
                        <div id="editInfoTabCB" class="tab-pane fade in active">
                            <div class="row">
                                ...
                                <div class="col-xs-12 col-md-6 modalCell">
                                    ...
                                    <div class="modalFieldLabelCnt">URI</div>
									<div id="inputUriCBMMsg" class="modalFieldMsgCnt">&nbsp;</div>
                                </div>
...
// end **MM 
				...
				"data-password": row.password
                        ...
                        onPostBody: function()
                        {
                            if(tableFirstLoad)
                            {
                                ...
                                var addDasboardDiv = $('<div class="pull-right"><i id="addContextBrokerBtn" data-toggle="modal" data-target="#addContextBrokerModal" class="fa fa-plus-square" style="font-size:36px; color: #ffcc00"></i></div>');
	...
	function drawMap1(latitude,longitude, flag){	
			...
			map.on("click", function (e) {
				 ...
				 document.getElementById('inputLatitudeCB').value = e.latlng.lat;
				 document.getElementById('inputLongitudeCB').value = e.latlng.lng;
				 if (marker){
			...
			map.on("click", function (e) {
				document.getElementById('inputLatitudeCBM').value = e.latlng.lat;
				document.getElementById('inputLongitudeCBM').value = e.latlng.lng;
				 if (marker){
					 map.removeLayer(marker);
				 }
				 marker = new L.marker([e.latlng.lat,e.latlng.lng]).addTo(map).bindPopup(e.latlng.lng, e.latlng.lng);

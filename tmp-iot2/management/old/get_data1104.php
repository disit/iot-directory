...
{
     $query2 = "SELECT value_name, data_type, value_type FROM event_values WHERE cb = '$cb' AND device = '$name'";
...
{
     $query2 = "SELECT value_type FROM value_types ORDER BY label";
     $res = mysqli_query($link, $query2) or die(mysqli_error($link));
     $labels = array();
    ...
    if(isset($_GET['action']) && !empty($_GET['action'])) 
    {
		...
		else if($action == 'get_param_device')//Escape 
        {
            ...
            if($r1) 
            {
				...
				$result['labels'] = generatelabels($link);
            } 
            else
            {
		...
		{
		   $result = array();
           $query  = "SELECT  d.uri, d.id, d.type AS entityType, d.kind, d.protocol,  d.longitude, d.latitude, d.contextBroker, cb.name, cb.protocol as type, cb.ip, cb.port FROM devices d JOIN contextbroker cb ON d.contextBroker = cb.name  ORDER BY id;";
		...
		{
		   $result = array();
           $query  = "SELECT v.cb, v.device, v.value_name, v.data_type, v.value_type, v.editable, v.value_unit, v.healthiness_criteria, d.latitude, d.longitude FROM `event_values` v JOIN devices d ON (v.device=d.id and v.cb = d.contextbroker)  ORDER BY v.value_name;";
           $r1 = mysqli_query($link, $query);

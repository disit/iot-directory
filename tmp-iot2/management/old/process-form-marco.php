   ...
   function checkLdapRole($connection, $userDn, $role) 
   {
    ...
    if(isset($_REQUEST['register_confirm']))
    {
        ...
        if(isset($_SESSION['loggedRole']))
        {
            ...
            {
                $insqDbtb = "INSERT INTO users(IdUser, username, password, name, surname, email, reg_data, status, ret_code, admin) VALUES (NULL, '$username', '$password', '$firstname', '$lastname', '$email', now(), 1, 1, '$valueAdmin')";
	...
	else if(isset($_POST['addContextBrokerConfirmBtn']))
    {
        ...
        // {
            ...
            $q = "INSERT INTO iotdirectorydb.contextbroker(name, ip, protocol, port, latitude, 
                        longitude, login, password) " .
                 "VALUES('$name', '$ip', '$protocol',  '$port', '$latitude', '$longitude', 
               '$login', '$password' )";
            $r = mysqli_query($link, $q);
            ...
            else 
            {
                ...
                // echo "Ko";
                 echo '<script type="text/javascript">';
                 echo 'alert("Error: An error occurred when registering the context broker. <br/>Please insert again!");';
                 echo '</script>';

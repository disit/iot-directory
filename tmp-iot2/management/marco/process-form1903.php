<?php
   ...
   function checkLdapRole($connection, $userDn, $role) 
   {
	...
	function registerKB($name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $listnewAttributes)
   {  
      /* msg for the Knowledge base */
      $msg=array();
      $msg["id"]=     $name;
      $msg["type"]=   $type;
      $msg["kind"]=    $kind;
      ...
      $msg_orion=array();
       ...
       /* msg for the orion context broker */
      $msg_orion["id"]=     $name; 
      $msg_orion["type"]=   $type;
      $msg_orion["geolocalization_lat"]=array();
      $msg_orion["geolocalization_lon"]=array();
      $msg_orion["geolocalization_lat"]["value"]=    $latitude;
      $msg_orion["geolocalization_lon"]["value"]=    $longitude;
      $msg_orion["geolocalization_lat"]["type"]= "Float";     
      $msg_orion["geolocalization_lon"]["type"]= "Float";
      $a=0;
         while ($a < count($listnewAttributes))
         {
                                  $att=$listnewAttributes[$a];
				 $msg_orion[$att->name]=array();
				 $msg_orion[$att->name]["value"]="0";
 				 $msg_orion[$att->name]["type"]=$att->val;
                                 $a++;
                              }
	...
	$url_orion="http://192.168.1.9:1026/v2/entities/";
        $options = array(
                  'http' => array(
                          'header'  => "Content-Type: application/json",
                          'header'  => "Access-Control-Allow-Origin: *",
	...
	//	$context_orion  = stream_context_create($options_orion);
          //         $result_orion = @file_get_contents($url_orion, true, $context_orion);
                   $context  = stream_context_create($options);
        ...
        {*/
            ...
            if($resultMaxSel2) 
            {
                ...
                if($type_widget == "widgetFirstAid")
                {
                    ...
                    $infoJsonObject = array();
                    $infoJsonFirstAxis = array();
                    $infoJsonSecondAxis = array();
    ...
    else if(isset($_REQUEST['modify_widget'])) 
    {
        ...
        if($type_widget_m == "widgetFirstAid")
        {
            ...
            $infoJsonObjectM = array();
            $infoJsonFirstAxisM = array();
            $infoJsonSecondAxisM = array();
	...
	else if(isset($_POST['addDevice']))
    {
           ...
           if (true)
        {
			...
			$longitude = mysqli_real_escape_string($link, $_POST['inputLongitudeDevice']);  
 			$listnewAttributes= json_decode($_POST['newattributesJson']);
                        // echo "marco prima";
			$uri= registerKB($name, $type, $contextbroker, $kind, $protocol, $format, $macaddress, $model, $producer, $latitude, $longitude,  $listnewAttributes); 
                
			
			$q = "INSERT INTO iotdirectorydb.devices(name, type, contextBroker,  kind, protocol, format, macaddress, model, producer, latitude, longitude,uri) " .
                 "VALUES('$name', '$type', '$contextbroker', '$kind', '$protocol', '$format', '$macaddress', '$model', '$producer', '$latitude', '$longitude', '$uri')";
            $r = mysqli_query($link, $q);
	    ...
	    echo $q;
            if($r) 
            {
				...
				{
				  $att=$listnewAttributes[$a];
				  $insertquery="INSERT INTO `attributes`(`cb`, `device`, `name`, `type`, `order`, `slabel`, `attvalues`) VALUES ('$contextbroker','$name','$att->name','$att->val','$b','','');";
				  //echo $insertquery;
	...
	else if(isset($_POST['updateDevice']))
    {
        ...
        if (true)
        {
            $nameM = mysqli_real_escape_string($link, $_POST['inputNameDeviceM']);
			$typeM = mysqli_real_escape_string($link, $_POST['inputTypeDeviceM']); 
			...
			// $uri= registerKB($nameM, $typeM, $contextbrokerM, $kindM, $protocolM, $formatM, "", $macaddressM, $modelM, $producerM, $latitudeM, $longitudeM); 
               
			$q = "UPDATE iotdirectorydb.devices SET type='$typeM', kind= '$kindM',contextBroker='$contextbrokerM', protocol='$protocolM', format='$formatM', macaddress='$macaddressM', model='$modelM', producer='$producerM', latitude='$latitudeM', longitude='$longitudeM' WHERE name='$nameM'";
            $r = mysqli_query($link, $q);
            ...
            if($r) 
            {
				...
				{
				  $att=$listnewAttributes[$a];
				  $insertquery="INSERT INTO `attributes`(`cb`, `device`, `name`, `type`, `order`, `slabel`, `attvalues`) VALUES ('$contextbrokerM','$nameM','$att->name','$att->val','$b','','');";
				  $r1 = mysqli_query($link, $insertquery);
	              // echo $insertquery;
				  $ok= $ok && $r1; 			
    			  $b++;
        ...
        // {
            ...
            $q = "INSERT INTO iotdirectorydb.contextbroker(name, ip, protocol, port, latitude, longitude, login, password) " .
	...
	else if(isset($_REQUEST['editContextBroker']))
    {
        ...
        if(isset($_SESSION['loggedRole']))
        {
            ...
            $q = "UPDATE iotdirectorydb.contextbroker SET name = '$name', uri = '$uri', ip = '$ip', protocol = '$protocol', created = '$created', latitude = '$latitude', longitude = '$longitude', login = '$login', password = '$password' WHERE name = '$name'";
            $r = mysqli_query($link, $q);

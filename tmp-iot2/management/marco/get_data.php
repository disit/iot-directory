...
{
     $query2 = "SELECT name, type, slabel FROM attributes WHERE cb = '$cb' AND device = '$name'";
...
{
     $query2 = "SELECT label FROM semanticlabel ORDER BY label";
     $res = mysqli_query($link, $query2) or die(mysqli_error($link));
     $labels = array();
...
if(isset($_SESSION['loggedRole']))
{
    if(isset($_GET['action']) && !empty($_GET['action'])) 
    {
		...
		else if($action == 'get_param_device')//Escape 
        {
            ...
            $result = array();
            $q1 = "SELECT * FROM devices WHERE name = '$name' and contextbroker = '$cb'";
            $r1 = mysqli_query($link, $q1);
            ...
            if($r1) 
            {
				...
				$result['attributes'] = generateAttributes($link,$name,$cb);
		...
		else if($action == 'get_param_contextbroker')//Escape 
        {
            $name = mysqli_real_escape_string($link, $_REQUEST['name']);
            $result = array();
            $q1 = "SELECT * FROM iotdirectorydb.contextbroker WHERE name = '$name'";
            $r1 = mysqli_query($link, $q1);
        ...
        {
  	        $q = "SELECT * FROM iotdirectorydb.contextbroker";
            $r = mysqli_query($link, $q);
            $result = array();
            if($r) 
            {

...
if (true)
{
    if(isset($_GET['action']) && !empty($_GET['action'])) 
    {
		...
		else if($action == 'get_param_device')//Escape 
        {
            $id = mysqli_real_escape_string($link, $_REQUEST['name']);
			$cb = mysqli_real_escape_string($link, $_REQUEST['contextBroker']);
			
            $result = array();
            $q1 = "SELECT * FROM devices WHERE id = '$id' and contextbroker = '$cb'";
            $r1 = mysqli_query($link, $q1);
            ...
            if($r1) 
            {
                ...
                $result['result'] = 'Ok';
                $result['deviceData'] = $row;
				$result['attributes'] = generateAttributes($link,$id,$cb);
				$result['value_type'] = generatelabels($link);
				$result['data-type'] = generatedatatypes($link);
				$result['units'] = generateunits($link);
				
            } 
            else
            {
                $result['result'] = 'Ko'; // . $q1 . mysqli_error($link);
            }
            echo json_encode($result);
            mysqli_close($link); 
        }
		else if($action == 'get_param_contextbroker')//Escape 
        {
            $name = mysqli_real_escape_string($link, $_REQUEST['name']);
            $result = array();
            $q1 = "SELECT * FROM contextbroker WHERE name = '$name'";
            $r1 = mysqli_query($link, $q1);
		...
		else if($action == 'get_contextbroker')//Escape 
        {
            $result = array();
            $q1 = "SELECT * FROM contextbroker";
            $r1 = mysqli_query($link, $q1);
		...
		else if ($action == 'get-config-data')
		{
		   $result = array();
           $query  = "SELECT  d.uri, d.id, d.devicetype AS entityType, d.kind, d.protocol,  d.longitude, d.latitude, d.contextBroker, cb.name, cb.protocol as type, cb.ip, cb.port FROM devices d JOIN contextbroker cb ON d.contextBroker = cb.name  ORDER BY id;";
           $r1 = mysqli_query($link, $query);
            ...
            {
                $result['result'] =  mysqli_error($link) . 'Ko';
            }
		...
		else if ($action == 'get-config-data-values')
		{
		   $result = array();
           $query  = "SELECT v.cb, v.device, v.value_name, v.data_type, v.value_type, v.editable, v.value_unit, v.healthiness_criteria, d.latitude, d.longitude, d.protocol FROM `event_values` v JOIN devices d ON (v.device=d.id and v.cb = d.contextbroker)  ORDER BY v.value_name;";
           $r1 = mysqli_query($link, $query);
		...
		else if($action == "getContextBroker")
        {
        ...
        else if($action == "getdatatypes")
        {
            echo json_encode(generatedatatypes($link));
        }
	else if($action == "getvaluetypes")
        {
            echo json_encode(generatelabels($link));
        }
	else if($action == "getvalueunits")
        {
            echo json_encode(generateunits($link));
        }

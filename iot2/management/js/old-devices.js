...
$.ajax({
  ...
  data:{
	...
	success: function (mydata) 
	{
		...
		{
			defaultPolicyValue["contextbroker"] = mydata["contextbroker"];
			defaultPolicyValue["protocol"] = mydata["protocol"];
			defaultPolicyValue["format"] = mydata["format"];
			defaultPolicyValue["healthiness_criteria"] = mydata["healthiness_criteria"];
			defaultPolicyValue["healthiness_value"] = mydata["healthiness_value"];									
		} 
	 
 },
 error: function (mydata)
		{
   console.log("Error reading default");  
...
function drawAttributeMenuUser
(attrName, data_type, value_type, parent)
{
    options="";
    if (value_type!="") labelcheck= value_type;
    else labelcheck="";	
 ...
 return "<div class=\"row\" style=\"border:3px solid blue;\" ><div class=\"col-xs-6 col-md-3 modalCell\">" +
		...
		"<div class=\"col-xs-6 col-md-3 modalCell\"><div class=\"modalFieldCnt\">" +
		//"<i class=\"fa fa-minus-square\" onclick=\"removeElementAt('" + parent + "',this); return true;\"  style=\"font-size:36px; color: #ffcc00\"></i></div></div></div>";
	    "<button class=\"btn btn-warning float-right\" onclick=\"removeElementAt('" + parent + "',this); return true;\">Remove Value</button></div></div></div>";
        ...
        function buildMainTable(destroyOld, selected=null)
        {
			...
			if (selected==null)
			{
			  mydata = {action: "get_all_device", token : sessionToken};
			}
			else
			{
	...
	$("#addMyNewDeviceConfirmBtn").click(function() {
		    ...
		    mynewAttributes = [];
			num1 = document.getElementById('addlistAttributesUser').childElementCount;
            for (var m=0; m< num1; m++)
			{	
			  var newatt= {value_name: document.getElementById('addlistAttributesUser').childNodes[m].childNodes[0].childNodes[0].childNodes[0].value.trim(), 
			            data_type:document.getElementById('addlistAttributesUser').childNodes[m].childNodes[1].childNodes[0].childNodes[0].value.trim(),
						value_type:document.getElementById('addlistAttributesUser').childNodes[m].childNodes[2].childNodes[0].childNodes[0].value.trim(), editable: 0, value_unit:"#", healthiness_criteria: "refresh_rate", healthiness_value : "300"};
					   if (newatt.value_name!="" && newatt.data_type!="" && newatt.value_type!="" && newatt.editable!="") mynewAttributes.push(newatt);
			}
            document.getElementById('addlistAttributesUser').innerHTML = "";	
						 ...
						 data:{
							  ...
							  id: $('#inputNameDeviceUser').val(),
							  type: $('#inputTypeDeviceUser').val(),
							  kind: $('#selectKindDeviceUser').val(),
							  latitude: $('#inputLatitudeDeviceUser').val(),
							  longitude: $('#inputLongitudeDeviceUser').val(),
							  mac: "",
							  model: "",
							  producer: "",
							  visibility: "private",
							  ...
							  // MM 2305
							  contextbroker : defaultPolicyValue.contextbroker,
							  protocol : defaultPolicyValue.protocol,
							  format : defaultPolicyValue.format,
							  token : sessionToken
							},
							 type: "POST",
							 async: true,
						...
						//addDeviceConditionsArray['addlistAttributes'] = true;
									...
									/* Delete lines related to attributes USER */

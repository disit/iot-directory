...
{
	$q = "SELECT v.*, d.kind, d.latitude, d.longitude, d.mandatoryproperties, d.mandatoryvalues FROM event_values v JOIN devices d ON (v.device=d.id AND d.contextbroker=v.cb)";
	$r = mysqli_query($link, $q);
	if($r) 
	{
		$result['status'] = 'ok';
...
else if($action == 'get_subset_event_value')
{
	...
	if (count($selection)!=0)
	{
		...
		$q = "SELECT v.*, d.kind, d.latitude, d.longitude, d.mandatoryproperties, d.mandatoryvalues FROM event_values v JOIN devices d ON (v.device=d.id AND d.contextbroker=v.cb) WHERE " . $cond;
		// echo $q;
	}
    else
...
{
	$q = "SELECT d.id, d.contextbroker, v.value_name, d.latitude, d.longitude  FROM devices d JOIN event_values v ON (d.id=v.device and d.contextbroker=v.cb)";
	$r = mysqli_query($link, $q);

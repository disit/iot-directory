...
/* ****FUNCTIONS FOR THE REGISTRATION OF A DEVICE IN THE CONTEXT BROKER AND IN THE KNOWLEDGE BASE ******  */
	...
	function insert_ngsi($name, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude,$visibility, $owner, $frequency, $listnewAttributes, $ip, $port, &$result)
	{
	   $res = "ok";
	   $msg_orion=array();
	...
	function insert_mqtt($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port, &$result)
	{
	  return "ok";
	}
	
	function insert_amqp($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port, &$result)
	{
	  return "ok";
	}

	function canBeModified($name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, &$result)
	{
	  $error=false;
	...
	function canBeRegistered($name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, &$result)
	{
	  $error=false;
	  if ($name==null || $name=="") { $error=true; $result["msg"].= "\n id not specified";}
          if ($contextbroker==null || $contextbroker=="") {$error=true; $result["msg"].= "\n cb not specified";}	  
	...
	function registerKB($link, $name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, &$result)
    {  
	  //$result=array();
      $result["status"]='ok';
      $result["msg"]='';	  
      if (canBeRegistered($name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $result))
      {	  
		...
		/* registration of the device in the corresponding context broker */
		 ...
		 if ($local_result!="errore")
		 {
			 ...
			 {
			   case "ngsi": 
					 $res = insert_ngsi($name, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port, $result);
					 break;
			   case "mqtt":
					$res = insert_mqtt($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port, $result);
					 break;
				case "amqp":	  
					$res = insert_amqp($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port, $result);
					 break;	 
			 }	
		 }		 
	...
	function update_ngsi($name, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, &$result)
	{
	   $res = "ok";
	   $msg_orion=array();
	...
	function update_mqtt($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri,&$result)
	{
	  return "ok";
	}
	
	function update_amqp($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri,&$result)
	{
	  return "ok";
	}
	
function updateKB($link, $name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $attributes, $uri, &$result)
{      
  // $result=array();
  $result["status"]='ok';
  $result["msg"]='';	  
        
  if (canBeRegistered($name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $attributes, $result))
  {	
	  $query="SELECT * from contextbroker WHERE name = '$contextbroker'";
	  $r = mysqli_query($link, $query);
	...
	{    
	  ...
	  {
	   case "ngsi": 
			 $res = update_ngsi($name, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude, $visibility, $owner, $frequency, $attributes, $ip, $port,$uri, $result);
			 break;
	   case "mqtt":
			$res = update_mqtt($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $attributes, $ip, $port,$uri, $result);
			 break;
		case "amqp":	  
			$res = update_amqp($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $attributes, $ip, $port,$uri,$result);
			 break;	 
	   }	 
	}
	if ($res=="ok") 
...
/* ****FUNCTIONS FOR THE DELETION OF THE REGISTRATION OF A DEVICE IN THE KNOWLEDGE BASE AND IN THE CONTEXT BROKER ****************** */			
	...
	function delete_ngsi($name, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, &$result)
	{
	   $res = "ok";
	...
	function delete_mqtt($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, &$result)
	{
	  return "ok";
	}
	
	function delete_amqp($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, &$result)
	{
	  return "ok";
	}
...
{  
	...
	$query  = "SELECT  d.uri, d.id, d.devicetype AS entityType, d.kind, d.format, d.macaddress, d.model, d.producer, d.protocol,  d.longitude, d.latitude, d.visibility, d.owner, d.frequency, cb.name, cb.protocol as type, cb.ip, cb.port, cb.login, cb.password, cb.latitude as cblatitude, cb.longitude as cblongitude, cb.created FROM devices d JOIN contextbroker cb ON d.contextBroker = cb.name WHERE d.contextBroker='$contextbroker' and d.id='$name';";
	...
	if (canBeModified($name, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $result))
	  {
		...
		if ($local_result!="errore") 
		 {
		   ...
		   {
		    case "ngsi": 
				 $res = delete_ngsi($name, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, $result);
				 break;
		    case "mqtt":
				$res = delete_mqtt($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, $result);
				 break;
			case "amqp":
                $res = delete_amqp($name, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, $result);
				break;	 
		  }
          if ($res=='ok')
          {
...
{  
	...
	$query  = "SELECT  d.uri, d.id, d.devicetype AS entityType, d.kind, d.format, d.macaddress, d.model, d.producer, d.protocol,  d.longitude, d.latitude, d.visibility, d.owner, d.frequency, cb.name, cb.protocol as type, cb.ip, cb.port, cb.login, cb.password, cb.latitude as cblatitude, cb.longitude as cblongitude, cb.created FROM devices d JOIN contextbroker cb ON d.contextBroker = cb.name WHERE d.contextBroker='$contextbroker' and d.id='$device';";
	...
	if (canBeModified($device, $type, $contextbroker, $kind, $protocol, $format,  $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $result))
	  {
		  /* msg for the Knowledge base + registration on the KB */
		 // echo "entrato";
		  $msg=array();
		...
		if ($local_result!="errore") 
		 {
		   ...
		   {
		    case "ngsi": 
				 $res = update_ngsi($device, $type, $contextbroker, $kind, $protocol, $format, $model, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, $result);
				 break;
		    case "mqtt":
				$res =update_mqtt($device, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, $result);
				 break;
			case "amqp":
                $res = update_amqp($device, $type, $contextbroker, $kind, $protocol, $format, $latitude, $longitude, $visibility, $owner, $frequency, $listnewAttributes, $ip, $port,$uri, $result);
				break;	 
		  }
          if ($res=='ok')
          {

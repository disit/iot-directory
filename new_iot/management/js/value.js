	...
	// var admin = "<?= $_SESSION['loggedRole'] ?>";
     ...
     //   setGlobals(admin, existingPoolsJson);
		...
		function buildMainTable(destroyOld, selected)
        {
            ...
            if (selected==null)
			{
			  mydata = {action: "get_all_event_value", token : sessionToken};
			}
			else
			{
                ...
                success: function (data)
                {
                    ...
                    $('#valuesTable').bootstrapTable({
                            ...
                            },
							{
								...
								return '<button type="button" id="active" class="btn btn-success"><span style="font-size:8px; color: white"></span></button>';
								else 
								return '<button type="button" id="iddle" class="btn btn-warning"><span style="font-size:10px; color: white"></span></button>';
                                 },
                                cellStyle: function(value, row, index, field) {
                                    if(index%2 !== 0)
                                    {
							...
							
                            {
                                ...
                                formatter: function(value, row, index)
                                { 
								   if (editButton && row.visibility!="delegated")
                            ...
                            },
                            {
                                ...
                                formatter: function(value, row, index)
                                {
                                    if (deleteButton && row.visibility!="delegated")
							...
							detailFormatter: function(index, row, element) {
                             var txt ="";
                            if (row.cb=="orionUNIFI") txt = ' IoT BRoker URI: ' + ' http://iot-app.snap4city.org/orion-broker/';
                            return txt + ' | Data Type: ' + data[index].data_type   + ' | Editable: ' + data[index].editable  + ' | k1: ' + row.k1 + ' | k2: ' + row.k2+ ' | Different Value: ' + data[index].different_values + " | Value Bound: " + data[index].value_bounds + " | Kind: " + data[index].kind;
							},

...
if ($action=="insert"){
	...
	for ($i = 0; $i < count($jsondata); $i++){
		...
		if (isset($device["edge_gateway_type"]))
				$edge_gateway_type = $device["edge_gateway_type"];
			else $edge_gateway_type="";
		
		if (isset( $device["edge_gateway_uri"]))
				 $edge_gateway_uri = $device["edge_gateway_uri"];
			else
				$edge_gateway_uri="";	

		if(isset($device["visibility"])){
		...
		//I don't use registerKB because I suppose that this check (if the contextBroker exists ecc) is made by the first phase
		//of upload
		...
		AND id = '$id';";
		//MM I have to guarantee that at current stage no devices with that name have ever inserted
		//	AND id = '$id' AND deleted IS null;";
	...
	//	$valuesKo=0;
		...
		if($s1 && $s2 && $s3){
			...
			$q = "INSERT INTO temporary_devices(username, contextBroker, devicetype, id, kind, macaddress,
				model,latitude, longitude,uri, protocol,producer, format, frequency, visibility,status, k1,k2,edge_gateway_type,edge_gateway_uri, validity_msg, organization)
				VALUES('$username','$contextbroker', '".$device["devicetype"]."', '$id','".($device["kind"])."',
				'".($device["macaddress"])."','".($device["model"])."','".($device["latitude"])."',
				'".($device["longitude"])."',
				'" . $result["content"] . "','".($device["protocol"])."','" .($device["producer"])."',
				'".($device["format"])."', '".($device["frequency"])."',
				'$visibility','".($device["status"])."',
				'".($device["k1"])."','".($device["k2"])."',
				'$edge_gateway_type','$edge_gateway_uri','".($device["validity_msg"])."', '$organization')";

				$r = mysqli_query($link, $q);
				if($r){
					//sara2210
					logAction($link,$usernameNotHashed,'temporary_devices','insert',$deviceName,$organization,'','success');
					...
					/*
					Sara2210
					$row["valuesOk"]=$valuesOk;
					 $row["valuesKo"]=$valuesKo;*/
					 $row["inserted"]="ok";
				...
				else 
				{
				   ...
				   logAction($link,$usernameNotHashed,'temporary_devices','insert',$deviceName,$organization,'Problem inserting the device','faliure');
		   ...
		   logAction($link,$usernameNotHashed,'temporary_devices','insert',$deviceName,$organization,'','faliure');
		   $row["inserted"]="ko";
		   array_push($resultInfo,$row);
		   continue;
		}	
...
else if ($action=="bulkload")
{
  ...
  echo_log("We reached the bulkLoad");
  ...
  $username = md5($username);
   ...
   //asynchBulk('../api/asynchBulk.php',array('username'=>$username,'link'=>$link, 'pathCertificate'=>$pathCertificate, 'accessToken'=>$accessToken));
	...
	echo_log($username);
	echo_log($organization);
    
    //---find total number of valid divices to be inserted-------
    $totalValid=0;
    $qcount = "SELECT count(*) FROM temporary_devices WHERE username = '$username'AND status='valid' AND organization= '$organization' AND should_be_registered='yes' AND  deleted IS null;";
    ...
    echo_log("total number of valid devices is "+ $totalValid);
    //-----------------------------------------------
    ...
    echo_log("update the bulk_status table that the bulk is processing".$b);
    //-----------------------------------------------
	...
	if($r) 
	{
        echo_log("all temporal rows are found");
		 while($row = mysqli_fetch_assoc($r)) 
		{	
             ...
             if($row['status']=='valid')
			  ...
			  $oidc = new OpenIDConnectClient($keycloakHostUri, $clientId, $clientSecret);
                    ...
                    echo_log("numberValidProcessed ".$numberValidProcessed);
					...
					{	
						...
						$accessToken,$result,'yes',$organization,$kbUrl);
					   ...
					   //Sara2210
						...
						if($result["status"]=="ok"){ // whenver the previous insert succeded
							...
							AND deleted is null AND should_be_registered='yes' AND status = 'valid'";
							...
							//Sara2210
							logAction($link,$usernameNotHashed,'device','bulkload',$deviceName,$organization,'','faliure');		echo_log("failure isterting ".$result["log"]);						   
							$rec["inserted"]="ko";
						}
					}
					else
...
{
    echo_log("inside get_bulk_status");
    $username = mysqli_real_escape_string($link,$_REQUEST['username']);
    $username=md5($username);
	echo_log("username is ".$username);
    ...
    $r = mysqli_query($link, $query);
     ...
     if($r)
	 {
         ...
         echo_log($row["is_bulk_processing"]);

...
{   
	...
	if($notDuplicate){
	updateKB($link, $id, $devicetype, $contextbroker, $kind, $protocol, $format, $macaddress, $model, $producer, $latitude, $longitude,$visibility, $frequency, $merge, $uri, $organization, $result); 
	...
	{
		$q = "UPDATE devices SET contextBroker='$contextbroker', devicetype='$devicetype', kind= '$kind', protocol='$protocol', format='$format', macaddress='$macaddress', model='$model', producer='$producer', latitude='$latitude', longitude='$longitude', frequency = '$frequency', organization='$organization' WHERE id='$id' and contextBroker='$old_contextbroker'";
	}
	else {			
		$q = "UPDATE  devices SET uri = '". $result["content"] . "', mandatoryproperties=1, mandatoryvalues=1, contextBroker='$contextbroker', devicetype='$devicetype', kind= '$kind', protocol='$protocol', format='$format', macaddress='$macaddress', model='$model', producer='$producer', latitude='$latitude', longitude='$longitude', frequency = '$frequency',organization='$organization'  WHERE id='$id' and contextBroker='$old_contextbroker'";
	}
...
else if($action == "get_device_value_type")
{ // Elf: this function is NOT to be included in the API 
    //Fatima: since it's not included, I will not update anyway
     
	$q = "SELECT d.id, ev.value_type, d.protocol, d.format,
       d.latitude AS dlat, d.longitude AS dlong, c.name,  c.ip, c.port, c.latitude, c.longitude FROM
	   devices d, contextbroker c, event_values ev WHERE ev.cb = d.contextbroker AND ev.device = d.id 
	   AND d.contextbroker = c.name AND
	   deleted is null and mandatoryproperties=1 and mandatoryvalues=1 order by d.id";
	...
	if($val){
	 ...
	 echo "value_type\t protocol\t format\t dlat\t dlong\t ip\t port\t latitude\t longitude\t";
	 ...
	 while($row = mysqli_fetch_assoc($r)) 
     {
	   echo $row["value_type"] . "\t" . $row["protocol"] . "\t" . $row["format"] . "\t" . $row["dlat"] . "\t" . $row["dlong"] . "\t" . $row["ip"] . "\t" . $row["port"] . "\t" . $row["latitude"] . "\t" . $row["longitude"] . "\t";

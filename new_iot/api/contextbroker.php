...
else if ($action == 'get_config_data')
{
   ...
   $r = mysqli_query($link, $q);
	...
	else if ($action=="insert_default_policy")
	...
	$q = "INSERT INTO defaultpolicy(policyname, active, contextbroker, protocol, format, healthiness_criteria, healthiness_value) " .
		 "VALUES('$name', '$a', '$cb', (SELECT protocol FROM contextbroker WHERE name=$cb), CASE WHEN protocol= \"mqtt\" THEN \"csv\" WHEN protocol= \"ngsi\" THEN \"json\" ELSE \"xml\" END , '$hc', '$hv' )";
	$r = mysqli_query($link, $q);
	...
	{
		 $res["status"]='ko';
		 $res["msg"] = '<script type="text/javascript">'.
						 'alert("Error: An error occurred when registering the context broker $name. <br/>' .
						   mysqli_error($link) . $q .
						   ' Please enter again the context broker")'. '</script>';
	}

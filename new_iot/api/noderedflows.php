...
{   
	...
	$sub_nature = mysqli_real_escape_string($link, $_REQUEST['sub_nature']);  
	$description = mysqli_real_escape_string($link, $_REQUEST['description']);  

	$q = "INSERT INTO `noderedflows`(`name`, `nature`, `sub_nature`, `description`)
	  VALUES ('$name','$nature','$sub_nature','$description');";	
	
	$r = mysqli_query($link, $q);
	if($r) 
	{
		$result["msg"] .= "\n Flow correctly inserted";
		$result["status"]='ok';   
	} 
	else 
	{
	   $result["status"]='ko';
	   $result["msg"] .= "\n Problem in inserting the flow $name:" . mysqli_error($link);  
	}
	echo json_encode($result);
	mysqli_close($link);
}
else if($action == "get_all_flows")
{
	$q = "SELECT `name`, `nature`, `sub_nature`, `description` FROM `noderedflows`";
	$r = mysqli_query($link, $q);
	...
	if($r) 
	{
     ...
     while($row = mysqli_fetch_assoc($r)) 
     {
	   ...
	   $result['status'] = 'ko';
	   $result['msg'] .= "\n Problem in reading the flows" . mysqli_error($link);
	}
	echo json_encode($result);
	mysqli_close($link);
}

...
function activateStub(button,cb,ip,port,protocol)
{
	...
	var data = "contextbroker=" + cb + "&ip=" + ip + "&port=" + port;
        var service = _serviceIP + "/stubs/"+protocol;
	console.log(data);
	console.log(service);
	var xhr = ajaxRequest();
...
{
    var service = _serviceIP + "/stubs/status";
	var xhr = ajaxRequest();
...
/*JQuery Started...*/
						...
						*/
                        ...
                        onPostBody: function()
                        {
							...
							var addMapDiv = $('<div id="displayDevicesMapCB" class="pull-left"><button type="button" class="btn btn-primary btn-round"><span class="glyphicon glyphicon-globe" style="font-size:36px; color: #0000ff"></span></button></div>');
								 ...
								 var addCBDiv = $('<div class="pull-right"><button id="addContextBrokerBtn"  class="btn btn-primary">New Contextbroker</button></div>');
	...
	function drawMap1(latitude,longitude, flag){	
			...
			map.on("click", function (e) {
				 ...
				 marker = new L.marker([lat,lng]).addTo(map).bindPopup(lng,lng);

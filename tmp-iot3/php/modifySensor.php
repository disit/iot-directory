...
if (!isset($_GET["errore"]))
{
	...
	$sens->fetchRecord(array("id"=>$_GET["id"]));
	if ($sens->error_status==1 && $sens->fetchNumRows()==1)
	{
	  $dati= $sens->result[0];
	}
	else
...
<form role="form" method="POST" action="php/modifySensor-exe.php">
...
<div class="form-group">
 ...
 <!-- `id`,`macaddress`,`type`,`model`,`producer`,`longitude`,`latitude`,`kind` set('sensor','actuator'),`protocol` 
   set('AMQP','MQTT','NGSI',''),`format` set('CSV','JSON','XML',''),

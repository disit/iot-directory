...
if ($action=="insert")
{
			...
			registerKB($link, $id, $devicetype, $contextbroker, $kind, $protocol, $format, $macaddress, $model, $producer, $latitude, $longitude, $visibility, $owner, $frequency, $listAttributes, $result); 
			
			if ($result["status"]=='ko') return $result;
			
			if ($result["status"]=='ok' &&  $result["content"]==null)
			{
			 $q = "INSERT INTO devices(id, devicetype, contextBroker,  kind, protocol, format, macaddress, model, producer, latitude, longitude, visibility, owner, frequency) " .
                 "VALUES('$id', '$devicetype', '$contextbroker', '$kind', '$protocol', '$format', '$macaddress', '$model', '$producer', '$latitude', '$longitude', '$visibility', '$owner', '$frequency')";
			}
			else {
			$q = "INSERT INTO devices(id, devicetype, contextBroker,  kind, protocol, format, macaddress, model, producer, latitude, longitude,uri, visibility, owner, frequency,mandatoryproperties,mandatoryvalues) " .
                 "VALUES('$id', '$devicetype', '$contextbroker', '$kind', '$protocol', '$format', '$macaddress', '$model', '$producer', '$latitude', '$longitude', '" . $result["content"] . "', '$visibility', '$owner', '$frequency',1,1)";
            }
...
{   
	...
	if ($listdeleteAttributes!=null) $merge= array_udiff($merge, $listdeleteAttributes, 'compare_values');
	 ...
	 updateKB($link, $id, $devicetype, $contextbroker, $kind, $protocol, $format, $macaddress, $model, $producer, $latitude, $longitude,$visibility, $owner, $frequency, $merge, $uri, $result); 
	...
	if ($result["status"]=='ok' &&  $result["content"]==null)
	{
		$q = "UPDATE devices SET devicetype='$devicetype', kind= '$kind', protocol='$protocol', format='$format', macaddress='$macaddress', model='$model', producer='$producer', latitude='$latitude', longitude='$longitude', frequency = '$frequency', visibility = '$visibility', owner='$owner' WHERE id='$id' and contextBroker='$contextbroker'";
	}
	else {			
		$q = "UPDATE  devices SET uri = '". $result["content"] . "', mandatoryproperties=1, mandatoryvalues=1, devicetype='$devicetype', kind= '$kind', protocol='$protocol', format='$format', macaddress='$macaddress', model='$model', producer='$producer', latitude='$latitude', longitude='$longitude', frequency = '$frequency', visibility = '$visibility', owner='$owner'  WHERE id='$id' and contextBroker='$contextbroker'";
	}
...
else if($action == "get_subset_device")
{
	...
	if (count($selection)!=0)
	{
		...
		$q = "SELECT DISTINCT * FROM devices WHERE " . $cond;
		// echo $q;
	}
    else
...
else if($action == "get_all_device_latlong")
{
	...
	$q = "SELECT id, contextBroker, latitude, longitude FROM devices";
	$r = mysqli_query($link, $q);
...
{
	$q  = "SELECT  d.uri, d.id, d.devicetype AS entityType, d.visibility, d.frequency, d.kind, d.protocol,  d.longitude, d.latitude, d.contextBroker, cb.name, cb.protocol as type, cb.ip, cb.port FROM devices d JOIN contextbroker cb ON d.contextBroker = cb.name  ORDER BY id;";
	$r = mysqli_query($link, $q);

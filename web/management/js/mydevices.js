...
$.ajax(
        {url: "../api/model.php",
            ...
            success: function (mydata) {
                if (mydata["status"] === 'ok') {
                    addModel2($("#selectModel"), mydata["data"], "");
...
//--------to get the ContextBroker items----------
$.ajax({
...
$.ajax({
    ...
    success: function (data) {
        if (data["status"] === 'ok') {
            ...
            addModel2($("#selectModel"), data["content"], "NATIVE");
        } else {
            console.log("error getting the context brokers " + data);
        }
    },
...
//CONTEXTBROKER AND PROTOCOL RELATION FOR ADD DEVICE -SELECTOR 
    $("#selectContextBroker").change(function () {
        ...
        /*  var index = document.getElementById("selectContextBroker").selectedIndex;
         ...
         if (valOrg != null)
         $("#selectContextBrokerMsg").html(" - Organization:" + valOrg);
         if (valkind == "external") {
...
//Start Related to Add Device 
    ...
    $("#selectModel").select2({width: '75%'});



    $("selectModel").val('').change();
...
// Device dataTable table Style 
    ...
    $("#addMyNewDeviceConfirmBtn").click(function () {
        ...
        if (nameOpt[selectednameOpt].value.includes('( NATIVE )')) {
            var mod = nameOpt[selectednameOpt].value.replace('( NATIVE )', '').trim();
        } else {
            var mod = nameOpt[selectednameOpt].value;
        }
    ...
    $("#selectModel").change(function () {
        ...
        } else
        {
            if (nameOpt[selectednameOpt].attributes.data_kind.value != "FIWIRE") {
                ...
                $('#selectContextBroker').prop('disabled', false);
                $('#KeyOneDeviceUser').val("");
                $('#KeyTwoDeviceUser').val("");
            ...
            if (nameOpt[selectednameOpt].attributes.data_subnature) {
                var subnature = nameOpt[selectednameOpt].attributes.data_subnature.value;
            } else {
                var subnature = '';
            }

            // var subnature = nameOpt[selectednameOpt].attributes.data_subnature.value ; //nameOpt[selectednameOpt].getAttribute("data_subnature")
            //add subnature + static info
            $("#selectSubnature").val(subnature);
            $('#selectSubnature').trigger('change');
            if (subnature != '') {
                subnatureChanged(false, JSON.parse(atob(nameOpt[selectednameOpt].attributes.data_static.value))); //  (nameOpt[selectednameOpt].getAttribute("data_static")
...
function drawMapAll_delegated(data, divName) {
    ...
    if (typeof map_all_delegated === 'undefined' || !map_all_delegated) {
        ...
        setTimeout(function () {
            map_all_delegated.invalidateSize()
        }, 400);
    }
}

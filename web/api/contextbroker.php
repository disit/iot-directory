...
{   
	...
	else 
	{
		...
		if (isset($_REQUEST['version'])) $version = mysqli_real_escape_string($link, $_REQUEST['version']);
		else $version = "";
		if (isset($_REQUEST['login'])) $login = mysqli_real_escape_string($link, $_REQUEST['login']);
		else $login = "";
		if (isset($_REQUEST['password'])) $password = mysqli_real_escape_string($link, $_REQUEST['password']);
		else $password = ""; 
		if (isset($_REQUEST['accesslink'])) $accesslink = mysqli_real_escape_string($link, $_REQUEST['accesslink']);
		else $accesslink = "";
		if (isset($_REQUEST['accessport'])) $accessport = mysqli_real_escape_string($link, $_REQUEST['accessport']);
		else $accessport = "";
		if (isset($_REQUEST['path'])) $path = mysqli_real_escape_string($link, $_REQUEST['path']);
		else $path="";
		if (isset($_REQUEST['visibility'])) $visibility = mysqli_real_escape_string($link, $_REQUEST['visibility']);
		else $visibility = "";
		if (isset($_REQUEST['sha'])) $sha = mysqli_real_escape_string($link, $_REQUEST['sha']);
		else $sha = "";
		if (isset($_REQUEST['urlnificallback'])) $urlnificallback = mysqli_real_escape_string($link, $_REQUEST['urlnificallback']);//TODO enforce mandatory for ngsi scenario
		else $urlnificallback = "";
		if (isset($_REQUEST['services'])) $services = json_decode($_REQUEST['services']);
		else $services = json_decode("[]");
		...
		{ 
				...
				if ($protocol == 'ngsi w/MultiService' && count($services) > 0)
				{
					...
					for($i = 0; $i < count($services); $i++)
					{
						$service = $services[$i];
						// syntax checking
						if(!preg_match($serviceRegex, $service)) $success = FALSE;
		...
		else
		{
				...
				if($r)
				{
					...
					if ($isEmpty)
					{
						$result["status"]="ko";
	    	            $result['msg'] = "Unrecognized context broker";
    	    	        $result["error_msg"] .= "Problem in delete context broker (Unrecognized context broker)";
...
else if($action == "get_delegations")
{
	...
	else 
	{
        ...
        if ($object=="BrokerID") $q ="SELECT organization FROM iotdb.contextbroker where name='$name'";
		else if ($object=="ModelID") $q ="SELECT organization FROM iotdb.model where name='$name'";
        $r = mysqli_query($link, $q);
        if($r)
        {
                while($row = mysqli_fetch_assoc($r))
...
{  
	...
	else 
	{
        ...
        //read broker info, it also enforce its existence
        if ($object=="BrokerID") $q ="SELECT organization FROM iotdb.contextbroker where name='$obj_name'";
		else if ($object=="ModelID") $q ="SELECT organization FROM iotdb.model where name='$obj_name'";
        $r = mysqli_query($link, $q);
        if($r)
        {
                while($row = mysqli_fetch_assoc($r))
...
else if ($action =='change_owner')
{
	...
	else 
	{
		...
		//read broker info, it also enforce its existence
        if ($object=="BrokerID") $q ="SELECT organization, accesslink FROM iotdb.contextbroker where name='$name'";
		else if ($object=="ModelID") $q ="SELECT organization FROM iotdb.model where name='$name'";
        $r = mysqli_query($link, $q);
        if($r)
        {
                ...
                {
                    $obj_organization =$row["organization"];
					if (isset($row["accesslink"])) $accesslink=$row["accesslink"];
                }
                ...
                else
                {
                    ...
                    else
                    {
				        ...
				        $ownmsg["elementName"]=$eId;				    
				        if (isset($accesslink))$ownmsg["elementUrl"]=$accesslink;
						else $ownmsg["elementUrl"]=$eId;

  ...
  mysqli_close($link);
    ...
    if (!empty($missingParams)) {
        ...
        if ($listAttributes == null)
            ...
            $merge = array_merge($listAttributes, $listnewAttributes, 'compare_values');
    ...
    $usernameNotHashed = $username;
    $username = md5($username);
    $name = mysqli_real_escape_string($link, $_REQUEST['id']);

    //alert the delete
    $qup = "UPDATE `iotdb`.`temporary_devices` SET `toDelete`='yes' WHERE `id`='$name' username = '$username'AND 
...
//echo json_encode($r);
    ...
    if ($r) {
        ...
        if ($row['status'] == 'valid') {
            ...
            if ($r1) {
                ...
                if ($result["status"] == "ok") { // whenver the previous insert succeded
                    logAction($link, $usernameNotHashed, 'device', 'singleload', $row["id"] . " " . $row["contextBroker"], $organization, $result["msg"], 'success');
                    $rec["inserted"] = "ok";
                    ...
                    if ($qdelete) {
                        logAction($link, $usernameNotHashed, 'temporary_devices', 'singleload deleted after insertion', $row["id"] . " " . $row["contextBroker"], $organization, '', 'success');
                        $result["status"] = 'ok';
                        $result["msg"] .= "\n  Device " . $row["id"] . " " . $row["contextBroker"] . " deleted from temporary_devices";
                        $result["log"] .= "\n  Device " . $row["id"] . " " . $row["contextBroker"] . " deleted from temporary_devices";
                        $rec["deleted"] = "ok";
                    } else {
                        logAction($link, $usernameNotHashed, 'temporary_devices', 'singleload update deleted', $row["id"] . " " . $row["contextBroker"],
                    ...
                    logAction($link, $usernameNotHashed, 'device', 'singleload', $row["id"] . " " . $row["contextBroker"], $organization, '', 'faliure');
                    $rec["inserted"] = "ko";
                }
            } else {
                logAction($link, $usernameNotHashed, 'device', 'singleload', '', $organization, 'Error: errors in reading data about devices.', 'faliure');
                $result['status'] = 'ko'; // . $q1 . generateErrorMessage($link);
                $result['msg'] .= 'CCC Error: errors in reading data about devices. <br/>' . generateErrorMessage($link);
                $result['log'] .= '\n\naction:singleload. Error: errors in reading data about devices. ' . generateErrorMessage($link);
                $rec["inserted"] = "ko";
            }
        ...
        logAction($link, $usernameNotHashed, 'device', 'singleload', '', $organization, '', 'faliure');
        $result['status'] = 'ko';
...
//TODO enforce presence parameters from node_data
    ...
    $kbUrl = mysqli_real_escape_string($link, $node_data->kbUrl);
    if (isset($node_data->should_be_registered))
        $should_be_registered = mysqli_real_escape_string($link, $node_data->should_be_registered); //can be yes or no, depending on internal or external scenario
    else

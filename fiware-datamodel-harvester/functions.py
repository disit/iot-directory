            ...
            if type(entry_list[_key]) in [dict, list]:
                if type(entry_list[_key]) is list:
                    ...
                    for i in range(len(entry_list[_key])):
                        temp_dict["_temp-" +
                                  str(iterator)] = entry_list[_key][i]
                        iterator += 1
                else:
                    temp_dict = entry_list[_key]
                path.append(_key)
                temp_res = self.find_key_from_dict(
                    target_key, delete, temp_dict)
...
out = find_extra_schema(
    "/home/giuseppe/PycharmProjects/auto_downl_ghub/SmartCities/dataModel.Building/")
resutl = dict()
...
def load_repository(link, repo_name="Repo"):
    ...
    for link in links:
        ...
        # Se non metti -1 resta il punto finale TODO
        Repo.clone_from(link, "./"+repo_name+"/" +
